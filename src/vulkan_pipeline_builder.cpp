...
namespace jre
{
    ...
    VulkanPipeline VulkanPipelineBuilder::Build()
    {
        ...
        assert(hinst && hwnd, "hinst and hwnd must be set");
    ...
    bool VulkanPipelineBuilder::Destroy(VulkanPipeline &pipeline)
    {
        pipeline.m_device.destroySemaphore(pipeline.m_image_available_semaphore);
        pipeline.m_device.destroySemaphore(pipeline.m_render_finished_semaphore);
        pipeline.m_device.destroyFence(pipeline.m_in_flight_fence);
        pipeline.m_device.destroyShaderModule(pipeline.m_vertext_shader->m_shader_module);
        pipeline.m_vertext_shader.reset();
        pipeline.m_device.destroyShaderModule(pipeline.m_fragment_shader->m_shader_module);
        pipeline.m_fragment_shader.reset();
        pipeline.m_device.destroyPipeline(pipeline.m_graphics_pipeline);
        pipeline.m_device.destroyPipelineLayout(pipeline.m_pipeline_layout);
        pipeline.m_device.destroyRenderPass(pipeline.m_render_pass);
        pipeline.m_device.destroyCommandPool(pipeline.m_command_pool);
        pipeline.DestroySwapChain();
        pipeline.m_device.destroy();
        pipeline.m_instance.destroySurfaceKHR(pipeline.m_surface);
        pipeline.m_instance.destroy();
        return true;
    }
    ...
    vk::SwapchainCreateInfoKHR VulkanPipelineSwapChainBuilder::Build(VulkanPipeline &pipeline)
    {
        ...
        debug::print("clamped current size: %d, %d\n", extend_width, extend_height);
